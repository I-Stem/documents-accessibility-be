const AuthChallengeResponse = require("../../models/auth-challenge-response.model");
...
const modelName = "auth-challenge-response";
...
exports.create = async (req, res, next) => {
    ...
    try {
        ...
        let result = await AuthChallengeResponse.create(data);
        if (result) {
...
exports.getAll = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let results = await AuthChallengeResponse.find().lean();
        if (results) {
...
exports.getById = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let result = await AuthChallengeResponse.findById(req.params.id).lean();
        if (result) {
            res.status(httpStatusCodes[200].code)
...
exports.update = async (req, res, next) => {
    ...
    try {
        ...
        let result = await AuthChallengeResponse.findByIdAndUpdate(req.params.id, data, { new: true });
        if (result) {
            res.status(httpStatusCodes[200].code)
                .json(formResponse(httpStatusCodes[200].code, result));
            return;
...
exports.delete = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let result = await AuthChallengeResponse.findByIdAndDelete(req.params.id);
        if (result) {
            res.status(httpStatusCodes[200].code)
