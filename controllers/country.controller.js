...
exports.getAll = async (req, res, next)=> {
    ...
    createLog(methodName, modelName);
    try {
        let results= await Country.find().lean();
        if (results) {
    ...
    exports.getById = async (req,res, next)=> {
        ...
        createLog(methodName, modelName);
        try {
            let result = await Country.findById(req.params.id).lean();
            if (result) {
                res.status(httpStatusCodes[200].code)
...
name: name.toLowerCase()
            ...
            let result = await Country.findByIdAndUpdate(req.params.id, data, {new: true});
            if (result) {
                res.status(httpStatusCodes[200].code)
                .json(formResponse(httpStatusCodes[200].code, result));
                return;
    ...
    exports.delete = async (req, res, next)=> {
        ...
        createLog(methodName, modelName);
        try {
            let result = await Country.findByIdAndDelete(req.params.id);
            if (result) {
                res.status(httpStatusCodes[200].code)
