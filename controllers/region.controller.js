...
exports.create = async (req, res, next)=> {
    ...
    try {
        ...
        let result = await Region.create(data);
        if (result) {
...
exports.getAll = async (req, res, next)=> {
    ...
    createLog(methodName, modelName);
    try {
        let results= await Region.find().lean();
        if (results) {
...
exports.getById = async (req,res, next)=> {
    ...
    createLog(methodName, modelName);
    try {
        let result = await Region.findById(req.params.id).lean();
        if (result) {
            res.status(httpStatusCodes[200].code)
...
exports.update = async (req, res, next)=> {
    ...
    try {
        ...
        let result = await Region.findByIdAndUpdate(req.params.id, data, {new: true});
        if (result) {
            res.status(httpStatusCodes[200].code)
            .json(formResponse(httpStatusCodes[200].code, result));
            return;
...
exports.delete = async (req, res, next)=> {
    ...
    createLog(methodName, modelName);
    try {
        let result = await Region.findByIdAndDelete(req.params.id);
        if (result) {
            res.status(httpStatusCodes[200].code)
