const UserQuestion = require("../../models/user-question.model");
...
exports.create = async (req, res, next) => {
    ...
    try {
        ...
        let result = await UserQuestion.create(data);
        if (result) {
...
exports.getAll = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let results = await UserQuestion.find().lean();
        if (results) {
...
exports.getById = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let result = await UserQuestion.findById(req.params.id).lean();
        if (result) {
            res.status(httpStatusCodes[200].code)
...
exports.update = async (req, res, next) => {
    ...
    try {
        ...
        let result = await UserQuestion.findByIdAndUpdate(req.params.id, data, { new: true });
        if (result) {
            res.status(httpStatusCodes[200].code)
                .json(formResponse(httpStatusCodes[200].code, result));
            return;
...
exports.delete = async (req, res, next) => {
    ...
    createLog(methodName, modelName);
    try {
        let result = await UserQuestion.findByIdAndDelete(req.params.id);
        if (result) {
            res.status(httpStatusCodes[200].code)
